...
def update_interaction_history(content: str, role: str, tool_context: ToolContext) -> dict:
    """
    Appends a new entry to the interaction history log. 
    Useful for summarizing sub-agent actions or recording final DM rulings into the persistent memory.
    
    Args:
        content: The text description of the event or dialogue.
        role: Who generated this? Usually "assistant" (for the DM) or "system" (for game events).
    """
    ...
    # 3. Append and save back to state
    ...
    return {
        ...
        "status": "success",
        "message": "Event successfully recorded in campaign history."
    }
...
dungeon_master_agent = Agent(
    ...
    Track all user (player) interactions in state['interaction_history'] and update the state using the update_interaction_history tool.
    ...
    **Interaction History:**
    ...
    tools=[update_location,update_player_name,update_interaction_history],
)
